    ...
    <div v-if="datasize>0" class="messageClass">
      ...
      >
        ...
        <el-table-column label="操作" width="230">
          <template slot-scope="props">
            <span @click.stop="agreeProjects(props.row)" v-if="props.row.state==4">同意</span>
            <span
              style="color:red"
              @click.stop="disagreeProject(props.row)"
              v-if="props.row.state==4"
            >拒绝</span>
            <span v-if="props.row.state==5">已同意</span>
            <span v-if="props.row.state==7" style="color:red">被拒绝</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
...
export default {
  ...
  methods: {
    //4代表导师邀请该学生，可进行同意/不同意操作
    //5代表老师以同意该学生参加
    //7代表学生被拒绝参加
    messages() {
      this.$api.get(
        "/api/v1/" + this.roles + "/project?state=4&state=5&state=7",
        {},
        res => {
